...
interface Estudante {
    ...
    matricula: string;
    turma_id: string;
    // Representing the contact struct the user implied (these properties might need adjusting if they are inside a sub-object in Firestore, e.g. pais.celularPai, but using root props to follow generic model)
    celular_responsavel?: string;
    telefone_responsavel?: string; // fallback if 'celular_responsavel' is empty
    celular_pai?: string;
    telefone_pai?: string; // fallback
    celular_mae?: string;
    telefone_mae?: string; // fallback 
}

interface ReportContatoPaisDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export function ReportContatoPaisDialog({ open, onOpenChange }: ReportContatoPaisDialogProps) {
    ...
    const fetchTurmas = async (ano: string) => {
        try {
            ...
            toast.error('Erro ao buscar turmas do ano selecionado');
        }
    };
    ...
    // Helper method to resolve contact
    const getContato = (cel?: string, tel?: string) => {
        if (cel) return cel;
        if (tel) return tel;
        return '-';
    }
                ...
                <div className="print-container p-8 bg-white text-black">
                    ...
                    <div className="border border-black p-4 mb-6">
                        ...
                        <div className="mt-4 bg-orange-100/50 p-2 border border-black text-center font-bold no-print-bg uppercase">
                            Relatório de Contato dos Pais/Responsáveis
                        </div>
                    </div>
                        ...
                        <table className="w-full border-collapse border border-black text-[13px]">
                            <thead>
                                <tr className="bg-gray-50 border border-black">
                                    <th className="border-r border-black p-2 w-10 text-center whitespace-nowrap">Nº</th>
                                    <th className="border-r border-black p-2 w-28 text-center">MATRÍCULA</th>
                                    <th className="border-r border-black p-2 text-left whitespace-nowrap">NOME DO(A) ESTUDANTE(A):</th>
                                    <th className="border-r border-black p-2 w-36 text-center">CONTATO RESPONSÁVEL</th>
                                    <th className="border-r border-black p-2 w-36 text-center">CONTATO DO PAI</th>
                                    <th className="p-2 w-36 text-center">CONTATO DA MÃE</th>
                                </tr>
                            </thead>
                                ...
                                {estudantes.map((estudante, index) => (
                                    <tr key={estudante.id} className="hover:bg-gray-50/50 border-b border-black">
                                        ...
                                        <td className="border-r border-black py-1 px-2 text-center">
                                            {getContato(estudante.celular_responsavel, estudante.telefone_responsavel)}
                                        </td>
                                        <td className="border-r border-black py-1 px-2 text-center">
                                            {getContato(estudante.celular_pai, estudante.telefone_pai)}
                                        </td>
                                        <td className="py-1 px-2 text-center">
                                            {getContato(estudante.celular_mae, estudante.telefone_mae)}
                                        </td>
                                    </tr>
                                ))}
