      ...
      try {
        ...
        if (!avData) {
          try {
            ...
            if (avSnap.exists()) {
              avData = { id: avSnap.id, ...avSnap.data() } as any;
            }
          } catch (avErr) {
        ...
        if (avData) {
          ...
          const objetos = avData.objetosConhecimento;
          if (Array.isArray(objetos)) {
            const cleaned = objetos
              .map(obj => typeof obj === 'string' ? obj.replace(/\s+/g, ' ').trim() : '')
              .filter(Boolean);
            topicoInicial = cleaned.join(", ");
          } else if (typeof objetos === 'string') {
            topicoInicial = objetos.replace(/\s+/g, ' ').trim();
          } else {
            topicoInicial = avData.titulo || "";
          }
        }
        ...
        if (!turmaData) {
          try {
            ...
            if (turmaSnap.exists()) {
              turmaData = { id: turmaSnap.id, ...turmaSnap.data() } as any;
            }
          } catch (turmaErr) {
        ...
        if (topicoInicial) {
          setAiConfig(prev => ({ ...prev, topico: topicoInicial }));
        }
